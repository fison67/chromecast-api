...
class Device extends EventEmitter {
      ...
      if('applications' in data){
          if(!this.player){
            this.client.launch(DefaultMediaReceiver, (err, player) => {
                this.player = player
                this._registerEventListener(player)
              })
           }
        }
       if('volume' in data){
          this.emit('volume', { level: data.volume.level, muted: data.volume.muted })
       }
    })
  ...
  close (callback) {
    ...
    try{ this.client.socket.destroy() }catch(err){}
    this.client.close()
    this.client = null
    this.player = null
    debug('Device closed')
